...
function Email({ setLogIn }) {
    ...
    const submitLogIn = () => {
        if (!message.passMess) {
            userService.postLogin(dataSend).then((data) => {
                if (!data?.data) {
                    ...
                    // linkRef.current.preventDefault();
    ...
    return (
        <div className={cx('signup-body')}>
            <form className={cx('form')}>
                ...
                {/* <Link to="/" ref={linkRef}> */}
                <Button to="/live" type="button" primary large className={cx('submit-btn')} onClick={submitLogIn}>
                    Log in
                </Button>
                {/* </Link> */}
            </form>
