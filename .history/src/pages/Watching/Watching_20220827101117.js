...
function Watching() {
    ...
    useEffect(() => {
        anUserService
            .getAnUser(nick)
            .then((data) => setAVideo(data.videos.find((video) => video.id === +idVideo)))
    ...
    return (
        <div className={cx('wrapper')}>
            <div className={cx('body-video')}>
                <video className={cx('video')} controls ref={videoRef} autoPlay loop>
                    <source src={aVideo.file_url} />
                </video>
                ...
                <div className={cx('cmt-on-mobile')} onClick={handlePlay}>
                    ...
                    <div className={cx('m-info')}>
                        <p className={cx('m-name')}> {aVideo.user.nickname}</p>
                        ...
                        {aVideo.music && (
                            <Link to="#" className={cx('m-music')}>
                                <BsMusicNoteBeamed /> {aVideo.music}
                            </Link>
                        )}
                    </div>
                    <div className={cx('m-action')}>
                        <Image src={aVideo.user.avatar} className={cx('m-avatar')} />
                        <div className={cx('m-like')}>
                            <div className={cx('m-like-body')}>
                                <AiTwotoneHeart className={cx('m-like-icon')} />
                            </div>
                            <span className={cx('m-like-count')}>{aVideo.likes_count}</span>
                            <div className={cx('m-like-body')}>
                                <FaCommentDots className={cx('m-like-icon')} />
                            </div>
                            <span className={cx('m-like-count')}> {aVideo.comments_count}</span>
                        </div>
            ...
            <div className={cx('author')}>
                <div className={cx('info')}>
                    <div className={cx('info-header')}>
                        <Link
                            to={{
                                pathname: '/@' + aVideo.user.nickname,
                                search: 'id=' + aVideo.user.id,
                            }}
                            className={cx('header-info-body')}
                        >
                            <Image src={aVideo.user.avatar} className={cx('avatar')} />
                            <div className={cx('info-wrapper')}>
                                <p className={cx('nickname')}>{aVideo.user.nickname}</p>
                                <p className={cx('name')}>
                                    {aVideo.user.first_name} <BsCheckSquare className={cx('icon-check')} /> - 1d ago
                                </p>
                            </div>
                        </Link>
                    ...
                    <div className={cx('action')}>
                        <div className={cx('like')}>
                            ...
                            <span className={cx('like-count')}>{aVideo.likes_count}</span>
                            <div className={cx('like-body')}>
                                <FaCommentDots className={cx('like-icon')} />
                            </div>
                            <span className={cx('like-count')}> {aVideo.comments_count}</span>
                        </div>
